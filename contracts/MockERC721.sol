...
contract QRC721 {
   ...
   modifier onlyOwner {
    ...
    function transferFrom(address senderAccountId, address receiverAccountId, uint256 tokenId) external {
        ...
        address thisOwner = QRC721.ownerOf(tokenId);
    ...
    function _transfer(address from, address to, uint256 tokenId) internal {
        require(QRC721.ownerOf(tokenId) == from, "Transfer from incorrect owner");
    ...
    function _approve(address to, uint256 tokenId) internal {
        ...
        emit Approval(QRC721.ownerOf(tokenId), to, tokenId);
