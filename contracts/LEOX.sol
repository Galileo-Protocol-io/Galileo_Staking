...
contract LEOX {
   ...
   * - `payeeId` cannot be the zero address.
   */
  function operatorTransfer(address payerId, address payeeId, uint256 amount) external returns (bool) {
   ...
   * - `payeeId` cannot be the zero address.
   */
  function operatorApprove(address payerId, address payeeId, uint256 amount) external returns (bool) {
   ...
   * - `payeeId` cannot be the zero address.
   */
  function operatorTransferFrom(address payerId, address payeeId, uint256 amount) external {
  ...
  function mint(address beneficiaryAccountId, uint256 amount, bytes calldata data) external onlyOwner returns (bool) {
    ...
    require(
      (_allowRemint && _maximumSupply >= (_totalSupply + amount)) ||
        (!_allowRemint && (_maximumSupply >= (_totalSupply + amount + _burnAccumulatedTotal))),
  ...
  function isOperatorFor(address AdditionalOwnerAccountId, address OwnerAccountId) public view returns (bool) {
    return AdditionalOwnerAccountId == OwnerAccountId || _accountOperators[OwnerAccountId][AdditionalOwnerAccountId];
  }
}
